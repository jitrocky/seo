...
st.write("输入URL或内容，AI帮您审计关键词、meta和意图匹配！基于最新趋势（AI Overviews + E-E-A-T）。支持中英混文。")
...
with tab1:
    ...
    if st.button("🚀 审计URL", type="primary") and url:
        try:
            headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'}  # 避Cloudflare
            response = requests.get(url, headers=headers)
            soup = BeautifulSoup(response.text, 'html.parser')
            
            title = soup.find('title').text.strip() if soup.find('title') else "无标题"
            meta_desc = soup.find('meta', attrs={'name': 'description'})
            desc = meta_desc['content'].strip() if meta_desc else "无描述"
...
with tab2:
    ...
    if st.button("🚀 审计内容", type="primary") and text:
        # 通用tokenization（支持中英：按空格/标点分词）
        # 中文：分汉字/英文词；英文：标准词
        words = re.split(r'\s+|[，。！？;:.()]+', text.lower())
        filtered_words = [w for w in words if len(w) > 1 and (w.isalpha() or any('\u4e00' <= c <= '\u9fff' for c in w))]  # 英+中
        freq = Counter(filtered_words)
        top_keywords = freq.most_common(5)
        
        # 意图得分 (0-10: 密度 + 问题词 + 来源)
        unique_kws = len(set([k for k, _ in top_keywords]))
        keyword_density = (unique_kws / len(set(filtered_words))) * 5 if filtered_words else 0
        has_question = bool(re.search(r'为什么|如何|什么|最佳|recommend|how|why|what', text, re.IGNORECASE))
        has_source = bool(re.search(r'根据|来源|data|according|source|参考', text, re.IGNORECASE))
        ...
        # 动态建议
        main_kw = top_keywords[0][0] if top_keywords else "主关键词"
        suggestions = [
            f"加问题句如'为什么选{main_kw}？'提升意图匹配。",
            "融入来源如'根据Yelp 2025数据'，加强E-E-A-T。",
            f"目标密度：主关键词'{main_kw}' 2-3%，长尾1%（AI Overviews友好）。"
        ]
        st.markdown("### 💡 优化建议")
        for sug in suggestions:
            st.write(f"- {sug}")
